...
output_dir = "/workspace/work/A2TG_Correct/FixedPC_mcmc_percentage_abla/MipNerf360"
...
grad_percentages = [0.25, 0.5, 0.75, 1.0]
scenes = []
...
for scene, point_count, grad_percentage in itertools.product(scenes, point_counts, grad_percentages):
    ...
    method_name = f'ntex_full_{int(grad_percentage*100.0)}'
    cmd_ntex = (
        f"CUDA_VISIBLE_DEVICES={CUDA_DEVICE_ID} python simple_trainer_textured_gaussians.py mcmc "
        f"{f"--ckpt {output_dir}/{method_name}/pc{point_count}/{scene}/ckpts/ckpt_29999.pt " if render else ""}"
        f"--data_dir {dataset_dir}/{scene} "
        ...
        f"--grad_percentage={grad_percentage} "
    ...
    method_name = f'ntex_noniso_{int(grad_percentage*100.0)}'
    cmd_ntex = (
        f"CUDA_VISIBLE_DEVICES={CUDA_DEVICE_ID} python simple_trainer_textured_gaussians.py mcmc "
        f"{f"--ckpt {output_dir}/{method_name}/pc{point_count}/{scene}/ckpts/ckpt_29999.pt " if render else ""}"
        f"--data_dir {dataset_dir}/{scene} "
        ...
        f"--disable_viewer "
        f"--data_factor {data_factor} "
        f"--min_aspect_ratio={100000000000000.0} "

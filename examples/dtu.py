...
for scene in scenes:
    cmd_2dgs = (
        ...
        f"--result_dir {output_dir}/2dgs_mcmc_{point_count}/{scene} "
        f"--dataset colmap "
        f"--init_extent 1 "
        f"--init_type sfm "
        f"--model_type=2dgs "
    ...
    cmd_textured_gaussians = (
        ...
        f"--data_dir {dataset_dir}/{scene} "
        f"--pretrained_path {output_dir}/2dgs_mcmc_{point_count}/{scene}/ckpts/ckpt_29999.pt "
        f"--result_dir {output_dir}/textured_gaussians_rgba_{point_count}/{scene} "
        f"--dataset colmap "

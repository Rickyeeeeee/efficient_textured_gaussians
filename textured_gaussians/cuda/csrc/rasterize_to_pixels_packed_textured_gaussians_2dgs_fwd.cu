...
__global__ void rasterize_to_pixels_fwd_packed_textured_gaussians_kernel(
    ...
    at::PackedTensorAccessor32<const S, 4, at::RestrictPtrTraits> textures_packed,    // [N, Texture_Resolution, Texture_Resolution, 4]
    ...
    for (uint32_t b = 0; b < num_batches; ++b) {
        ...
        for (uint32_t t = 0; (t < batch_size) && !done; ++t) {
            ...
            int32_t vcoords[4];
            S bilerp_weights[4];
            int32_t valid_texture = compute_bilinear_coords_weights(s.x, s.y, 16, 16, ucoords, vcoords, bilerp_weights);
            ...
            if (valid_texture > 0) {
                for (uint32_t i = 0; i < 4; ++i) {
                    alpha_scaling_factor += bilerp_weights[i] * textures_packed[3][offsets + ucoords[i] * 16 + vcoords[i]];
                }
            } else {
            ...
            for (uint32_t k = 0; k < COLOR_DIM; ++k) {
                ...
                if (valid_texture > 0) {
                    for (uint32_t i = 0; i < 4; ++i) {
                        tex_color += bilerp_weights[i] * textures_packed[k][offsets + ucoords[i] * 16 + vcoords[i]];
                    }
                }
...
call_kernel_with_dim(
    ...
    if (cudaFuncSetAttribute(
            rasterize_to_pixels_fwd_packed_textured_gaussians_kernel<CDIM, float>,
...
rasterize_to_pixels_fwd_packed_textured_gaussians_tensor(
    ...
    const torch::Tensor &colors,    // [C, N, channels] or [nnz, channels]
    const torch::Tensor &opacities, // [C, N]  or [nnz]
    const torch::Tensor &textures_packed,  // [channels, ...]
    const torch::Tensor &texture_dims, // 
    const torch::Tensor &texture_offsets, //
